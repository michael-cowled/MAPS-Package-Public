...
validate_and_get_paths <- function(folder) {
  ...
  path_annotations_new <- paste0(folder, "/mzmine/data_annotations_new.csv")
  ...
  if (file.exists(path_annotations_new)) {
    ...
    error_mzmine_annotations <- "The preferred file 'data_annotations_new.csv' was selected but is missing from the mzmine folder."
    message("Note: Using the preferred annotation file: 'data_annotations_new.csv'")
  } else {
    ...
    error_mzmine_annotations <- paste(
      ...
      "The preferred file 'data_annotations_new.csv' was not found."
    )
    message("Note: Using the fallback annotation file: 'data_annotations.csv' (if it exists). 'data_annotations_new.csv' was not found.")
  }
